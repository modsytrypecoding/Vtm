...
public class sendCorrect2 implements CommandExecutor{
	String Correct1 = Main.getconfig().getString("settings.Questions.Question2.CorrectAwnser2");
	String False1 = Main.getconfig().getString("settings.Questions.Question2.FalseAwnser1");
	String False2 = Main.getconfig().getString("settings.Questions.Question2.FalseAwnser2");
	...
	public boolean onCommand(CommandSender sender, Command command, String lable, String[] args) {
		if(sender instanceof Player) {
			...
			if(args.length == 0) {
				...
				p.sendMessage("                            ");
				p.sendMessage("§c[§6" + SecondNPCBuilder.NAMENPC2 + "§c]: §aRichtig! §rKommen wir nun zur nächsten Frage...");
				p.sendMessage("                            ");
				p.sendMessage("§c[§6" + SecondNPCBuilder.NAMENPC2 + "§c]: §6" + Main.getconfig().getString("settings.Questions.Question2"));
				tc1.setText("§c[§a" + Correct1 + "§c]" + " ");
				tc2.setText("§c[§a" + False1 + "§c]" + " ");
				tc3.setText("§c[§a" + False2 + "§c]" + " ");
				tc1.setClickEvent(new ClickEvent(Action.RUN_COMMAND, "/sendCorrect3"));
